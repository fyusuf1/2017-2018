...
public class MenuState extends BasicGameState {
    ...
    private static final String[] credits = {"Credits", "Code and TiledMap: Miikka Merikanto, miikka.merikanto@businesscollege.fi", "Player's images (CC3.0): cjc83486 from opengameart.com, http://opengameart.org/users/cjc83486", "CC3.0 reserved"};
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
        ...
        if (input.isKeyPressed(Input.KEY_ENTER)) {
            switch (playersChoice) {
                case CONTINUE:
                    sbg.enterState(Aemora.GAMEPLAYSTATE);
                    break;
                case SAVE:
                    Playerpos = ((GamePlayState) sbg.getState(Aemora.GAMEPLAYSTATE)).getPlayerPosition();
                    fileio.save(Playerpos);
                    sbg.enterState(Aemora.GAMEPLAYSTATE);
                    break;
                case LOAD:
                    Playerpos = fileio.loadSave();
                    ((GamePlayState) sbg.getState(Aemora.GAMEPLAYSTATE)).setPlayerPosition(Playerpos);
                    sbg.enterState(Aemora.GAMEPLAYSTATE);
                    break;
                case QUIT:
                    gc.exit();
